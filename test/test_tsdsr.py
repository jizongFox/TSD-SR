...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help="path to the pretrained sd3",
    )
    parser.add_argument(
        "--lora_dir",
        type=str,
        default="checkpoint/tsdsr",
        help="path to tsd-sr lora weights",
    )
    parser.add_argument(
        "--embedding_dir",
        type=str,
    ...
    parser.add_argument(
        "--output_dir", "-o", type=str, default="outputs/", help="path to save results"
    )
    parser.add_argument(
        "--input_dir",
        "-i",
    ...
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="recursively scan input_dir when it is a folder",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--is_use_tile", type=bool, default=False, help="whether to use tiled vae"
    )
    parser.add_argument(
        "--vae_decoder_tiled_size",
        type=int,
        default=224,
        help="tiled size for tiled vae decoder",
    )
    parser.add_argument(
        "--vae_encoder_tiled_size",
        type=int,
        default=1024,
        help="tiled size for tiled vae encoder",
    )
    parser.add_argument(
        "--latent_tiled_size",
        type=int,
        default=64,
        help="tiled size for transformer latent",
    )
    parser.add_argument(
        "--latent_tiled_overlap",
        type=int,
        default=8,
        help="tiled overlap for transformer latent",
    )
    ...
    parser.add_argument(
        "--process_size", type=int, default=512, help="process size for images"
    )
    parser.add_argument(
...
def collect_image_paths(input_dir, recursive=False):
    ...
    if recursive:
        ...
        for filename in os.listdir(input_dir):
            ...
            if (
                ...
                and os.path.splitext(filename)[1].lower() in valid_ext
            ):
...
def _gaussian_weights(tile_width, tile_height, nbatches):
    ...
    return torch.tile(
        torch.tensor(weights, device=args.device),
        (nbatches, transformer.config.in_channels, 1, 1),
    )
...
def tile_sample(
    ...
    with torch.no_grad():
        ...
        if h * w <= tile_size * tile_size:
            ...
            print(
                f"[Tiled Latent]: the input size is {lq.shape[-2]}x{lq.shape[-1]}, need to tiled"
            )
            ...
            for row in range(grid_rows):
                ...
                for col in range(grid_cols):
                    ...
                    if len(input_list) == 1 or col == grid_cols - 1:
                        input_list_t = torch.cat(input_list, dim=0).to(
...
def main(args, pixel_values, size):
    with torch.no_grad():
        ...
        model_input = (
            vae.encode(pixel_values).latent_dist.sample() * vae.config.scaling_factor
        )
        ...
        # Decode the output
        image = (
            vae.decode(latent_stu / vae.config.scaling_factor, return_dict=False)[0]
            .squeeze(0)
            .clamp(-1, 1)
        )
...
if __name__ == "__main__":
    ...
    vae_lora_state_dict = StableDiffusion3Pipeline.lora_state_dict(
        args.lora_dir, weight_name="vae.safetensors"
    )
    ...
    prompt_embeds = torch.load(
        os.path.join(args.embedding_dir, "prompt_embeds.pt"), map_location=args.device
    ).to(dtype=weight_dtype)
    pooled_prompt_embeds = torch.load(
        os.path.join(args.embedding_dir, "pool_embeds.pt"), map_location=args.device
    ).to(dtype=weight_dtype)
    ...
    if os.path.isdir(args.input_dir):
        ...
        image_names = collect_image_paths(args.input_dir, recursive=args.recursive)
    else:
    ...
    for image_name in tqdm(image_names):
        ...
        lr_scale = lr.resize(
            (int(ori_width * args.upscale), int(ori_height * args.upscale))
        )
        pixel_values = (
            tensor_transforms(lr).unsqueeze(0).to(args.device, dtype=weight_dtype)
        )
        ...
        if resize_flag:
            image_pil_image = image_pil_image.resize(
                (int(ori_width * args.upscale), int(ori_height * args.upscale))
            )
        ...
        if args.preserve_dir_structure and input_root is not None:
            rel_path = os.path.relpath(os.path.abspath(image_name), input_root)
            save_path = os.path.join(args.output_dir, rel_path)
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
        else:
            save_path = os.path.join(args.output_dir, os.path.basename(image_name))

        image_pil_image.save(save_path)
